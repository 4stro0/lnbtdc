    ...
    run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
      try{
        if (args[0]) {
          ...
          if (cmd.aliases) embed.addField("**Aliases**", `\`${cmd.aliases}\`` || "No Aliases");
          if (!cmd.aliases) embed.addField("**Aliases**", `\`No Aliases\``)
          if (cmd.aliases === "") embed.addField("**Aliases**", `\`No Aliases\``)
          ...
          if (cmd.memberpermissions) embed.addField("**Permission**", `\`${cmd.memberpermissions}\`` || "Public Command")
          if (!cmd.memberpermissions) embed.addField("**Permission**", `\`Public Command Anyone Can Use This\`` || "Public Command")
          return message.reply({embeds: [embed.setColor(ee.color)]});
        } else {
          const embed = new MessageEmbed()
              .setColor(ee.color)
              .setThumbnail(client.user.displayAvatarURL())
              .setTitle("HighQ Help Commands")
              .setFooter(`To see Commmand Information, type: ${prefix}help [CMD NAME] To See Music Command List Type \`-helpmusic\``, client.user.displayAvatarURL());
