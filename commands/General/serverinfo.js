  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    try {
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .addField("❱ Total HUMANS", "👤 \`" + message.guild.members.cache.filter(member => !member.bot).size + "\`", true)
        .addField("❱ Total BOTS", "🤖 \`" + message.guild.members.cache.filter(member => member.bot).size + "\`", true)
